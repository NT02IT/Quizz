...
public class LoginController {
	...
	void SignUp_Login(ActionEvent event) throws IOException {
		...
		if (!CheckForm.checkEmail(textField_Email_SignUp_Login.getText())) {
			Notification.Error(Notification.Error, "Email không đúng");
			return;
		}
		if (!CheckForm.isVietnamesePhoneNumber(textField_Phone_SignUp_Login.getText())) {
			Notification.Error(Notification.Error, "SDT không đúng");
			return;
		}
		...
		if (!create_Account) {
			Notification.Error(Notification.Error, "Tạo tài khoản thất bại");
			return;
		}
