...
public class GroupController implements Initializable {
	...
	public void loadData_Group() {
		...
		ComboBoxGroup.setOnAction(envent -> {
			...
			studentList = gr.getStudentsFromGroup2(Group_Current);
	...
	public void saveStudent_Group(ActionEvent event) {
		try {
			...
			Boolean Confirm = Notification.Comfrim(Notification.Default,
					"Học sinh này đã có trong nhóm khác! Bạn muốn thêm học sinh vào nhóm?")
		...
		try {
			...
			if (isChange) {
				for (Student s : studentList) {
					if (s.getStudentID().equals(student_Current.getStudentID())) {
						s = student_Change;
						break;
					}
				}

				Notification.Infomation(Notification.Successfully, "Thông tin học sinh thay đổi!");
			}
		} catch (Exception e) {
			e.printStackTrace();
			Notification.Error(Notification.Unsuccessfully, "Thông tin học sinh không được thay đổi!");
		}
	}
	...
	public boolean checkFill_Save_AddGroup() {
		return textField_GroupID.getText() != null && textField_GroupName.getText() != null
	...
	void save_AddGroup_Group(ActionEvent event) {
		...
		try {
			if (!gr.addGroup(
				...
				Notification.Error(Notification.Unsuccessfully, "Group không được tạo");
				return;
			}
	...
	void deleteGroup_Group(ActionEvent event) {
		...
		ComboBoxGroup.setValue(null);
		tableView_Group.getItems().clear();
	}

	@FXML
	private boolean Import_Group(ActionEvent event) throws SQLException {
			...
			try {
				...
				if (!check_FormatExel) {
					Notification.Error(Notification.Error,
							"File exel không họp lệ hoặc chưa chọn file exel");
					return false;
				}
